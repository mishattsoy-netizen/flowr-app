...
export function FolderView({ entity, isLoading }: FolderViewProps) {
      ...
      <div className="flex-1 overflow-y-auto px-8 py-8 flex flex-col h-full bg-background select-none items-center">
        <div className={cn("w-full flex flex-col gap-4 flex-1 min-h-0", isFullWidth ? "w-full" : "max-w-[1200px] mx-auto")}>
                ...
                {!isLoading && (
                  <Tooltip content="Rename">
                    <button
                      ...
                      className={`inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-[var(--radius-medium)] text-muted-foreground transition-opacity duration-200 ease-in-out ${(editingEntity?.id === entity.id && editingEntity.source === 'view') ? 'opacity-0 pointer-events-none' : 'hover:opacity-100 opacity-60'}`}
                    >
          ...
          >
            ...
            {isLoading ? (
              <div className="flex gap-4 pb-1.5 pr-10 overflow-hidden w-full">
                {[1, 2, 3, 4].map(i => (
                  <Skeleton key={i} className="w-[240px] shrink-0 h-full rounded-2xl bg-[var(--bone-5)]" />
                ))}
              </div>
                ...
                <div className="flex-1 overflow-y-auto no-scrollbar">
                  {isLoading ? (
                    <div className="flex flex-col gap-0.5">
                      {[1, 2, 3, 4, 5].map(i => (
                        <Skeleton key={i} className="h-[38px] w-full rounded-[var(--radius-medium)] bg-[var(--bone-5)]" />
                      ))}
                    </div>
                            ...
                            <div className="flex items-center gap-3 shrink-0">
                              ...
                              <button
                                ...
                                className={cn(
                                  ...
                                  contextMenu?.entityId === item.id && "opacity-100 text-foreground bg-[var(--sidebar-hover)]"
                                )}
                              >
                                <MoreHorizontal className="w-4 h-4 text-[var(--bone-70)]" />
                              </button>
                            </div>
                          </div>
                        );
