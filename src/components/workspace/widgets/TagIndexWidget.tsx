  ...
  return (
    <div className="flex flex-col h-full bg-sidebar group/widget rounded-[var(--radius-big)] overflow-hidden shadow-sm transition-all">
      <div className="px-5 py-4 border-b border-[var(--bone-5)] flex items-center justify-between bg-[var(--color-panel)]/50 backdrop-blur-sm">
        <h3 className="font-widget-header text-[15px] font-semibold text-[var(--bone-60)] group-hover/widget:text-foreground">Tag Index</h3>
      </div>
      <div className="flex-1 overflow-y-auto p-4 scrollbar-thin">
        {tags.length === 0 ? (
          <div className="text-sm text-[var(--bone-60)]">No tags found.</div>
        ) : (
          <div className="flex flex-wrap gap-2">
            {tags.map(t => (
              <span
                ...
                title={`${t.count} items`}
              >
                #{t.tag} <span className="ml-1 text-[var(--bone-60)]">({t.count})</span>
              </span>
            ))}
          </div>
        )}
