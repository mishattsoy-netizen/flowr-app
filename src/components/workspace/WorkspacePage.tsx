...
export function WorkspacePage({ entity }: { entity: Entity }) {
  ...
  const recentEntities = useMemo(() => {
    ...
    const list = recentEntityIds
      ...
      .filter((e): e is Entity => !!e && (e.workspaceId === entity.id || descendantIds.has(e.id)) && (e.type === 'note' || e.type === 'canvas'))
      .slice(0, 10);
  ...
  const handleCardClick = (targetEntity: Entity) => {
    if (targetEntity.workspaceId) {
      setActiveWorkspaceId(targetEntity.workspaceId);
    }
              ...
              <div className="relative">
                <button
                  onMouseDown={(e) => {
                    e.stopPropagation();
                    const rect = e.currentTarget.getBoundingClientRect();
                    setNewItemPopupPos({ x: rect.left - 120, y: rect.bottom + 8 });
                  }}
