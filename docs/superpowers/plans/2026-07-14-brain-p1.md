...
export async function addBrainNode(
  ...
  // Budget pre-check (spec §5): projected total must stay under the limit.
  const cfg = await getBrainConfigForUser(userId)
  const current = await compileBrain(userId)
  const addedCost = estimateTokens(content ?? label ?? '') || 50 // refs estimated after resolve; 50 = floor
  if (current.tokenCount + addedCost > cfg.token_limit && !pinned) {
