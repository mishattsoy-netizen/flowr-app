...
- **Write path:** the `isDesktop()` subscriber's existing `saveEntityToFile(entity, blocks)` call is replaced with `window.flowrDB.upsertEntity(entityRow)` for note/canvas entities (same trigger condition: `lastModified` changed, `syncMode !== 'cloud-only'`). This call is synchronous inside the main process and always succeeds locally regardless of network state. The subscriber pattern is extended to also cover `tasks` and `spaces` (Section 1), which today have no equivalent desktop-local persistence at all — this spec adds that.
  - **`syncMode === 'cloud-only'` entities are deliberately excluded from the SQLite mirror**, same as they're excluded from the file-vault today — they exist only in Supabase and arrive locally via `mergeCloudData`/the bulk setters, never written to disk. This is intentional (that's what "cloud-only" means) and must not be "fixed" by widening the subscriber's filter.
...
- **Pull (Supabase → local), and avoiding the re-push echo:** `mergeCloudData` (`SupabaseProvider.tsx`) applies incoming remote data via the **bulk setters** `setEntities`/`setTasks`/`setSpaces` (`store.ts:234`, `:527`, `:3241`) — not via the per-mutation actions (`addEntity`, `updateEntityContent`, etc.) that contain the Supabase push calls. **This is the actual invariant that keeps pull echo-free: the bulk setters never call `upsertEntity`/`upsertTask`/`upsertSpace`, so a pull can never re-trigger a Supabase push, regardless of what the SQLite subscriber does.** The `isDesktop()` subscriber (path 2) does observe the `lastModified` changes from a bulk-setter pull and correctly writes them through to SQLite via `flowrDB.upsertEntity` — that's desired (SQLite should reflect merged state) and harmless, since the SQLite subscriber was never wired to Supabase in the first place. **This invariant must be preserved when implementing the debounce below:** the debounce wrapper must be added at the existing per-mutation call sites, not as a new subscriber watching `lastModified` — a `lastModified`-watching push subscriber would fire on bulk-setter pulls too (it can't tell "user edited" from "just pulled" apart, since the `dirty`-flag mechanism that could have distinguished them was deliberately dropped in Section 2), reintroducing the exact echo this design avoids.
...
- **Push (local → Supabase):** each of the ~13 existing per-mutation call sites in `store.ts` (path 1 in Section 2) that call `upsertEntity(entity)`/`upsertTask(task)`/`upsertSpace(space)` directly is changed to call a new `debouncedPush(entity)` wrapper (one per table, keyed by row id) instead — a thin, drop-in replacement at the call site, not a new subscriber. `debouncedPush` collapses rapid edits (typing) into one real Supabase call per id per 1.5s window, then calls the existing `upsertEntity`/etc. **`debouncedPush` is only ever invoked from these action call sites — never from `setEntities`/`setTasks`/`setSpaces`, preserving the pull-echo invariant below.** No other behavior changes on the Supabase side — call targets and payloads (`entityToRow` etc.) are unchanged.
- **Pull (Supabase → local):** the existing Supabase Realtime subscription per authenticated user (already implemented in `sync.ts`/`SupabaseProvider.tsx`) continues to drive `mergeCloudData`, comparing `remote.lastModified` vs local `lastModified` (both stored as `last_modified` epoch-ms — there is no separate server `updated_at` clock in the current schema, so this spec does not introduce one). See Section 2 for why this never echoes back to Supabase.
