...
| 6 | Memory v2 (§7) | 🔀 **Superseded (2026-07-14)** — memory cards, fact sheet, and `manage_memory` are replaced by the Brain (`2026-07-14-brain-design.md`); idle-run auto-capture moves to Brain P3+. Brain **P1 shipped (2026-07-16)** — Tasks 1-9 done. Live acceptance tests 1 (injection/cutover) and 2 (cache) verified against real transcripts + OpenRouter logs; tests 3-7 not individually run (owner decision — confidence extended from 1-2 passing). Settings UI's stale 20-memory hard cap also found+fixed during this pass. ⬅️ **NEXT**: Brain P2a (multi-brain presets, spec `2026-07-16-brain-presets-design.md`) code-complete pending live verification. |
...
- ~~Token accounting now prefers real provider-reported `usage.prompt_tokens`/`completion_tokens` over the chars-based estimate when available.~~ **Reverted same day — see "Follow-up" below.** `token_usage_total` (the compaction-trigger metric) now always uses the history-based estimate; `providerUsage` remains correctly used elsewhere for cost accounting/analytics, untouched.
...
- **Migration applied to the live DB** (owner confirmed 2026-07-14): `supabase/migrations/20260714_compaction_watermark.sql` ran successfully.
- **Live-tested by owner same day** — this is what surfaced the token-usage-metric regression documented in the follow-up note below. A full "watermark advances across two real compactions" trace is still not explicitly collected, but the trigger/metric path has now been exercised live, including the fix verification.
