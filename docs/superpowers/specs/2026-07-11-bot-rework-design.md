...
| 6 | Memory v2 (§7) | 🔀 **Superseded (2026-07-14)** — memory cards, fact sheet, and `manage_memory` are replaced by the Brain (`2026-07-14-brain-design.md`); idle-run auto-capture moves to Brain P3+. Brain P1 code-complete (2026-07-16): Tasks 1-8/9 done (schema, compiler, store, prompt injection, `manage_brain` tool, settings UI, API route, Brain page). `tsc`/`vitest` verified clean (459/459). ⬅️ **NEXT: Task 9** — live acceptance tests only (owner-executed, see Brain spec §9). See Brain spec §10. |
