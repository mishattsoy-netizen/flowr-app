...
### 6c. Focus tracking — 🔴 AUTOMATIC DETECTION RETIRED (2026-07-14), tool kept for explicit use
...
**What was removed:** the classifier's `focus_shift` field and its instruction block (`classifier_v2.txt`, `V2Classification` in `routerV2.ts`, `parseClassifierV2Output`), and the auto-detection block in `chainRouter.ts` that wrote `current_focus`/cleared `pending_action` based on it. `tools.txt` Rule 12 rewritten to say `update_focus` should ONLY be called on an explicit user request ("let's focus on X"), not inferred. `system_prompt.txt`'s `[FOCUS]`-beats-history carve-out is left in place (harmless, and correct if `[FOCUS]` is ever populated via an explicit call) but is now rarely load-bearing since `[FOCUS]` will rarely be populated.

**What was kept:** the `update_focus` tool itself, `current_focus`/`previous_focus` columns, and the `[FOCUS]` prompt injection — all now purely for the case where a user explicitly says "let's focus on X" or similar. This is a narrower, deliberate bookmark, not an automatic tracker. **Explicitly NOT a fix for the underlying "bot loses my intent" complaint** — that problem is still open and unmeasured; retiring a non-working mechanism removes noise but doesn't itself improve intent-tracking. Next steps for that: (1) reproduce the note-transcript degradation pattern with a real transcript before proposing a fix (best current hypothesis: `[CURRENT CONTEXT]`'s DATE/TIME block dominating a short follow-up message — same "scaffolding crowds out real speech" class as §6d, unverified), (2) separately, `§7b` (compaction) is a plausible but unconfirmed contributor for longer conversations specifically — do not assume it explains a short-conversation degradation without checking.

Verified: `npx tsc --noEmit` clean, 150/150 tests passing in `src/lib/bot` after the retirement (down from 158 — 8 focus_shift-specific tests removed along with the feature).

> Found in the same 2026-07-13 investigation. The same transcript showed a second, independent failure: mid-conversation the bot lost track of what topic was currently active and needed the user to re-explain, even though nothing had been compacted or dropped from history (§7b's context-hole bug does not apply here — the conversation was short). This is a reasoning/state-tracking gap, not a missing-context gap: the bot had no explicit signal for "what am I currently doing," so it had to re-infer intent from raw history every turn, and sometimes inferred wrong. This remains the open problem after focus-tracking's retirement — see "What was kept" above for next steps.

**Design:**

- New columns on `bot_session_states`: `current_focus TEXT`, `previous_focus TEXT`.
- New tool `update_focus({ focus: string })` (added to `FLOWR_TOOLS` in `src/lib/bot/tools/definitions.ts`, handler in `src/lib/bot/tools/handlers.ts`) — writes `current_focus` → `previous_focus`, new value → `current_focus`, via `updateSessionState(context.sessionId, { current_focus, previous_focus })`. Not a mutating/user-content tool, so it's deliberately excluded from `MUTATING_TOOLS` in `outputGuard.ts` — it never needs grounding-guard treatment.
- System prompt instructs the model to call `update_focus` whenever the user's intent meaningfully shifts to a new subject (so the old focus is preserved in `previous_focus` rather than silently discarded), and again whenever the user explicitly returns to something earlier (so `current_focus` snaps back without the model having to re-derive it from raw history). This is intentionally lightweight — not called on every trivial turn, only on genuine topic boundaries.
- Both values are injected into `dynamicContext` every turn: `[FOCUS]\nCurrent: <current_focus>\nPrevious: <previous_focus>` (omitted entirely if both are null — a fresh session has nothing to inject).
- **Interaction with §6b:** an `update_focus` call to a genuinely new topic implicitly clears any stale `pending_action` in the same turn (if the user changed the subject, an old unconfirmed delete must not execute later just because a bare "yes" shows up in an unrelated context).
- Session-scoped only, same lifecycle as the rest of `bot_session_states` — a new `activeChatId` (from `/new` or a new web chat) gets a fresh row by construction; nothing needs explicit clearing.

**Acceptance:** user researches topic A, then says "actually, let's do X instead" → bot's `[FOCUS]` block updates, does not mix A's context into X. User later says "back to what we were doing" → bot correctly resumes A using `previous_focus`, not a guess from raw history.
