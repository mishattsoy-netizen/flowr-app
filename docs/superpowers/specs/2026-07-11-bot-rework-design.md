...
**Session descriptions** (from `session-descriptions-idea.md`): the same auto-capture run emits a second output — a ≤300-char description of what the session was about, stored on the session row, deleted with the session, refreshed only if the session resumes and goes idle again. Enables cross-session awareness ("what was I working on in session X?") and later, browsable sessions via `list_content`. No per-turn update overhead.
