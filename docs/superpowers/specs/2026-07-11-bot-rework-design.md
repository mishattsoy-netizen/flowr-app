...
| 6 | Memory v2 (§7) | 🔀 **Superseded (2026-07-14)** — memory cards, fact sheet, and `manage_memory` are replaced by the Brain (`2026-07-14-brain-design.md`); idle-run auto-capture moves to Brain P3+. Brain P1 in progress: Tasks 1-5/9 done (schema, compiler, store, prompt injection, `manage_brain` tool + `manage_memory` retirement). `bot_memories` no longer written to anywhere in the pipeline. ⬅️ **NEXT: Task 6** (settings UI repoint) — delegated. See Brain spec §10. |
